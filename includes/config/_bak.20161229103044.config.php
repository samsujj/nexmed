<?php
	...
	$AI->set_setting( 'SITE_BLURB_KEY', '[[SITE_BLURB_KEY]]' );
	...
	switch( AI_SITE_CONFIG_MODE )
	{
		case 'aicore':
		{
			...
			$AI->set_setting( 'DB_USERNAME', '[[DB_USERNAME]]' );
			$AI->set_setting( 'DB_BLURP', '[[DB_PASSWORD]]' );
			$AI->set_setting( 'DB_DATABASE', '[[DB_DATABASE]]' );
			
			//!!!!!!! ALSO SET RewriteBase IN .htaccess !!!!!!!!!!
			//URLS (Full URL and Path to application with trailing slash)
			$AI->set_setting( 'DEFAULT_HTTP_URL', '[[DEFAULT_HTTP_URL]]' );
			$AI->set_setting( 'DEFAULT_HTTPS_URL', '[[DEFAULT_HTTPS_URL]]' );
      		//!!!!!!! ALSO SET RewriteBase IN .htaccess !!!!!!!!!!
